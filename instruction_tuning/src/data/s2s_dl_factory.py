        ...
        def compute_metrics(eval_preds: EvalPrediction):
            ...
            for batch_preds, batch_labels in tqdm(
                ...
                padded_preds, padded_labels = pad_tensors_to_same_length(
                    batch_preds, batch_labels, constant_values=tokenizer.pad_token_id
                )
                for metric_name, metric_fn in metric_funcs.items():
                    result = metric_fn(
                        padded_preds=padded_preds, padded_labels=padded_labels, pad_token_id=tokenizer.pad_token_id
                    )
...
def padded_sequence_accuracy(
    *, padded_preds: np.ndarray = None, padded_labels: np.ndarray = None, pad_token_id: int = 0
) -> float:
    weights = np.not_equal(padded_labels, pad_token_id).astype(np.float32)
...
def padded_precision(
    *, padded_preds: np.ndarray = None, padded_labels: np.ndarray = None, pad_token_id: int = 0,
    ...
    weights = np.not_equal(padded_preds, pad_token_id).astype(np.float32)
    padded_preds = padded_preds.astype(np.int64)
    padded_labels = padded_labels.astype(np.int64)
...
def padded_f1(
    *, padded_preds: np.ndarray = None, padded_labels: np.ndarray = None, pad_token_id: int = 0,
    ...
    recall = padded_recall(padded_preds=padded_preds, padded_labels=padded_labels, pad_token_id=pad_token_id)
    precision = padded_precision(padded_preds=padded_preds, padded_labels=padded_labels, pad_token_id=pad_token_id)
